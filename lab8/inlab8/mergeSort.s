...
mergeSort:
	...
	jge	done		;if (start >= end), do nothing
	mov r10, rsi	;move start into r10, use r10 as local variable for mid
	add	r10, rdx	;start + end
	sar r10, 1		;divide previous result by 2
	push rsi		;push start to stack
	push r10		;push mid to stack
	push rdx		;push end to stack
	mov rdx, r10	;put mid in rdx (3rd parameter)
	call mergeSort	
	pop rdx			;pop end
	pop r10			;pop mid
	pop rsi			;pop start
	push r10		;push mid to stack
	add r10, 1		;add 1 to mid for merge
	push rsi		;push start on stack
	push rdx		;push end on stack
	mov rsi, r10	;move mid+1 into 2nd parameter
	call mergeSort
	pop rcx			;pop end directly to 4th parameter
	pop rsi			;pop mid directly to 3rd parameter
	pop rdx			;pop start directly to 2nd parrameter
	call merge
